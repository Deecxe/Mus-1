...
public class Partie {
  ...
  public static class Score {
    ...
    private final Map<Equipe, Integer> manchesGagneesParJoueur = new HashMap<>();

    public Score(Opposants opposants) {
      this.manchesGagneesParJoueur.put(opposants.equipe1(), 0);
      this.manchesGagneesParJoueur.put(opposants.equipe2(), 0);
    }
    ...
    public Optional<Equipe> vainqueur() {
      return manchesGagneesParJoueur.keySet().stream()
        .filter(equipe -> manchesGagneesParJoueur.get(equipe) == NB_MANCHES_A_GAGNER).findAny();
    }
  }
